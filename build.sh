...
# Please check Pathless documentation for more info
#  https://github.com/Tron-Z/pathless_system
...
if [[ "${PATHLESS_ENABLE_CALL_TRACING}" == "yes" ]]; then
...
if [[ -f "${SRC}"/scripts/general.sh ]]; then
	...
	echo "Please clone the full repository by https://github.com/Tron-Z/pathless_system"
	exit 255
...
# Purge Pathless Docker images
if [[ "${1}" == dockerpurge && -f /etc/debian_version ]]; then
	display_alert "Purging Pathless Docker containers" "" "wrn"
	docker container ls -a | grep pathless | awk '{print $1}' | xargs docker container rm &> /dev/null
	docker image ls | grep pathless | awk '{print $3}' | xargs docker image rm &> /dev/null
	shift
	set -- "docker" "$@"
fi
