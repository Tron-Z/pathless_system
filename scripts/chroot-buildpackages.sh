...
create_build_script ()
{
	...
	# set local version
	# debchange -l~pathless${REVISION}-${builddate}+ "Custom $VENDOR release"
	debchange -l~pathless"${REVISION}"+ "Custom $VENDOR release"
...
chroot_installpackages()
{
	...
	[[ -f ${SDCARD}/etc/hostapd.conf ]] && sed -i "s/^ssid=.*/ssid=Pathless/" ${SDCARD}/etc/hostapd.conf
} #############################################################################
