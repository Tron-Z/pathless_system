...
create_desktop_package ()
{
	...
	Section: xorg
	Priority: optional
	Recommends: ${DEBIAN_RECOMMENDS//[:space:]+/,}, pathless-bsp-desktop
	Provides: ${CHOSEN_DESKTOP}, pathless-${RELEASE}-desktop
	Pre-Depends: ${PACKAGE_LIST_PREDEPENDS//[:space:]+/,}
	Description: Pathless desktop for ${DISTRIBUTION} ${RELEASE}
	EOF
	...
	# Pathless create_desktop_package scripts

	unset aggregated_content

	mkdir -p "${destination}"/etc/pathless

	local aggregated_content=""
	aggregate_all_desktop "pathless/create_desktop_package.sh" $'\n'
...
create_bsp_desktop_package ()
{
	...
	# set up control file
	cat <<-EOF > "${destination}"/DEBIAN/control
	Package: pathless-bsp-desktop-${BOARD}
	Version: $REVISION
	Architecture: $ARCH
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Installed-Size: 1
	Section: xorg
	Priority: optional
	Provides: pathless-bsp-desktop, pathless-bsp-desktop-${BOARD}
	Depends: ${BSP_CLI_PACKAGE_NAME}
	Description: Pathless Board Specific Packages for desktop users using $ARCH ${BOARD} machines
	EOF
	...
	local aggregated_content=""
	aggregate_all_desktop "debian/pathless-bsp-desktop/postinst" $'\n'
	...
	local aggregated_content=""
	aggregate_all_desktop "debian/pathless-bsp-desktop/prepare.sh" $'\n'
	eval "${aggregated_content}"
...
desktop_postinstall ()
{
	...
	if [[ $LINUXFAMILY == sun8i && $BRANCH == legacy ]]; then
		...
		echo "disp_mem_reserves=on" >> "${SDCARD}"/boot/pathlessEnv.txt
		echo "extraargs=cma=96M" >> "${SDCARD}"/boot/pathlessEnv.txt
	fi

	if [[ -d ${SDCARD}/etc/lightdm ]]; then
		mkdir -p ${SDCARD}/etc/lightdm/lightdm.conf.d
		cat <<-EOF > ${SDCARD}/etc/lightdm/lightdm.conf.d/22-pathless-autologin.conf
		[Seat:*]
		autologin-user=$PL_USERNAME
