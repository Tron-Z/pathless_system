...
PRE_INSTALL_DISTRIBUTION_SPECIFIC
	...
	if [[ ${RELEASE} == "raspi" ]]; then
		...
		# install locally built packages or install pre-built packages from pathless
...
create_rootfs_cache()
{
	...
	if [[ -f $cache_fname && "$ROOT_FS_CREATE_ONLY" != "force" ]]; then
		...
		display_alert "Upgrading base packages" "Pathless" "info"
		...
		display_alert "Installing the main packages for" "Pathless" "info"
		eval 'LC_ALL=C LANG=C chroot $SDCARD /bin/bash -e -c "DEBIAN_FRONTEND=noninteractive apt-get -y -q \
			$apt_extra $apt_extra_progress --no-install-recommends install $PACKAGE_MAIN_LIST"' \
			${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/${LOG_SUBPATH}/debootstrap.log'} \
			${OUTPUT_DIALOG:+' | dialog --backtitle "$backtitle" --progressbox "Installing Pathless main packages..." $TTY_Y $TTY_X'} \
			${OUTPUT_VERYSILENT:+' >/dev/null 2>/dev/null'} ';EVALPIPE=(${PIPESTATUS[@]})'

		[[ ${PIPESTATUS[0]} -ne 0 ]] && exit_with_error "Installation of Pathless main packages for ${BRANCH} ${BOARD} ${RELEASE} ${DESKTOP_APPGROUPS_SELECTED} ${DESKTOP_ENVIRONMENT} ${BUILD_MINIMAL} failed"
		...
		if [[ $BUILD_DESKTOP == "yes" ]]; then
			...
			display_alert "Installing the desktop packages for" "Pathless" "info"
			eval 'LC_ALL=C LANG=C chroot $SDCARD /bin/bash -e -c "DEBIAN_FRONTEND=noninteractive apt-get -y -q \
				$apt_extra $apt_extra_progress install ${apt_desktop_install_flags} $PACKAGE_LIST_DESKTOP"' \
				${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/${LOG_SUBPATH}/debootstrap.log'} \
				${OUTPUT_DIALOG:+' | dialog --backtitle "$backtitle" --progressbox "Installing Pathless desktop packages..." $TTY_Y $TTY_X'} \
				${OUTPUT_VERYSILENT:+' >/dev/null 2>/dev/null'} ';EVALPIPE=(${PIPESTATUS[@]})'

			[[ ${PIPESTATUS[0]} -ne 0 ]] && exit_with_error "Installation of Pathless desktop packages for ${BRANCH} ${BOARD} ${RELEASE} ${DESKTOP_APPGROUPS_SELECTED} ${DESKTOP_ENVIRONMENT} ${BUILD_MINIMAL} failed"
		fi
		...
		[[ ${EVALPIPE[0]} -ne 0 ]] && exit_with_error "Installation of Pathless packages failed"

		# stage: purge residual packages
		display_alert "Purging residual packages for" "Pathless" "info"
		...
		eval 'LC_ALL=C LANG=C chroot $SDCARD /bin/bash -e -c "DEBIAN_FRONTEND=noninteractive apt-get -y -q \
			...
			${PROGRESS_LOG_TO_FILE:+' | tee -a $DEST/${LOG_SUBPATH}/debootstrap.log'} \
			${OUTPUT_DIALOG:+' | dialog --backtitle "$backtitle" --progressbox "Purging residual Pathless packages..." $TTY_Y $TTY_X'} \
			${OUTPUT_VERYSILENT:+' >/dev/null 2>/dev/null'} ';EVALPIPE=(${PIPESTATUS[@]})'

		[[ ${EVALPIPE[0]} -ne 0 ]] && exit_with_error "Purging of residual Pathless packages failed"
...
PREPARE_IMAGE_SIZE
	...
	exec {FD}> /var/lock/pathless-debootstrap-losetup
	flock -x $FD
	...
	if [[ -f $SDCARD/boot/pathlessEnv.txt ]]; then
		if [[ $CRYPTROOT_ENABLE == yes ]]; then
			echo "rootdev=$rootdevice cryptdevice=UUID=$(blkid -s UUID -o value ${LOOP}p${rootpart}):$ROOT_MAPPER" >> $SDCARD/boot/pathlessEnv.txt
		else
			echo "rootdev=$rootfs" >> $SDCARD/boot/pathlessEnv.txt
		fi
		echo "rootfstype=$ROOTFS_TYPE" >> $SDCARD/boot/pathlessEnv.txt
	...
	# if we have boot.ini = remove pathlessEnv.txt and add UUID there if enabled
	if [[ -f $SDCARD/boot/boot.ini ]]; then
		...
		if [[ $LINUXFAMILY != meson64 ]]; then
			[[ -f $SDCARD/boot/pathlessEnv.txt ]] && rm $SDCARD/boot/pathlessEnv.txt
		fi
	fi

	# if we have a headless device, set console to DEFAULT_CONSOLE
	if [[ -n $DEFAULT_CONSOLE && -f $SDCARD/boot/pathlessEnv.txt ]]; then
		if grep -lq "^console=" $SDCARD/boot/pathlessEnv.txt; then
			sed -i "s/^console=.*/console=$DEFAULT_CONSOLE/" $SDCARD/boot/pathlessEnv.txt
		else
			echo "console=$DEFAULT_CONSOLE" >> $SDCARD/boot/pathlessEnv.txt
		fi
	fi
