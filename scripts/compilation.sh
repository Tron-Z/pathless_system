...
compile_uboot()
{
	...
	while read -r target; do
		...
		eval CCACHE_BASEDIR="$(pwd)" env PATH="${toolchain}:${toolchain2}:${PATH}" \
			...
			# pathless specifics u-boot settings
			[[ -f .config ]] && sed -i 's/CONFIG_LOCALVERSION=""/CONFIG_LOCALVERSION="-pathless"/g' .config
		...
		[[ -n $UBOOT_TOOLCHAIN2 ]] && cross_compile="PATHLESS_BUILD=dummy"; # empty parameter is not allowed
	...
	Section: kernel
	Priority: optional
	Provides: pathless-u-boot
	Replaces: pathless-u-boot
	Conflicts: pathless-u-boot, u-boot-sunxi
...
compile_firmware()
{
	...
	plugin_dir="pathless-firmware${FULL}"
	...
	# cp : create hardlinks
	cp -af --reflink=auto "${EXTER}"/cache/sources/pathless-firmware-git/* "${firmwaretempdir}/${plugin_dir}/lib/firmware/"
	...
	Package: pathless-firmware${FULL}
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Installed-Size: 1
	Replaces: linux-firmware, firmware-brcm80211, firmware-ralink, firmware-samsung, firmware-realtek, pathless-firmware${REPLACE}
	Section: kernel
	Priority: optional
	...
	# pack
	mv "pathless-firmware${FULL}" "pathless-firmware${FULL}_${REVISION}_all"
	display_alert "Building firmware package" "pathless-firmware${FULL}_${REVISION}_all" "info"
	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "pathless-firmware${FULL}_${REVISION}_all" >> "${DEST}"/${LOG_SUBPATH}/install.log 2>&1
	mv "pathless-firmware${FULL}_${REVISION}_all" "pathless-firmware${FULL}"
	rsync -rq "pathless-firmware${FULL}_${REVISION}_all.deb" "${DEB_STORAGE}/"
...
compile_pathless-zsh()
{

	local tmp_dir pathless_zsh_dir
	tmp_dir=$(mktemp -d)
	chmod 700 ${tmp_dir}
	...
	[[ $IGNORE_UPDATES != yes ]] && fetch_from_repo "${PATHLESS_OHMYZSH_REPO:-${PATHLESS_SYSTEM_REPO}}" "${EXTER}/cache/sources/oh-my-zsh" "${PATHLESS_OHMYZSH_BRANCH:-branch:oh-my-zsh}"
	[[ $IGNORE_UPDATES != yes ]] && fetch_from_repo "${PATHLESS_EVALCACHE_REPO:-${PATHLESS_SYSTEM_REPO}}" "${EXTER}/cache/sources/evalcache" "${PATHLESS_EVALCACHE_BRANCH:-branch:evalcache}"

	mkdir -p "${tmp_dir}/${pathless_zsh_dir}"/{DEBIAN,etc/skel/,etc/oh-my-zsh/,/etc/skel/.oh-my-zsh/cache}

	# set up control file
	cat <<-END > "${tmp_dir}/${pathless_zsh_dir}"/DEBIAN/control
	Package: pathless-zsh
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Depends: zsh, tmux
	Section: utils
	Priority: optional
	Description: Pathless improved ZShell
	END

	# set up post install script
	cat <<-END > "${tmp_dir}/${pathless_zsh_dir}"/DEBIAN/postinst
	#!/bin/sh
	...
	cp -R "${EXTER}"/cache/sources/oh-my-zsh "${tmp_dir}/${pathless_zsh_dir}"/etc/
	cp -R "${EXTER}"/cache/sources/evalcache "${tmp_dir}/${pathless_zsh_dir}"/etc/oh-my-zsh/plugins
	cp "${tmp_dir}/${pathless_zsh_dir}"/etc/oh-my-zsh/templates/zshrc.zsh-template "${tmp_dir}/${pathless_zsh_dir}"/etc/skel/.zshrc

	chmod -R g-w,o-w "${tmp_dir}/${pathless_zsh_dir}"/etc/oh-my-zsh/

	# we have common settings
	sed -i "s/^export ZSH=.*/export ZSH=\/etc\/oh-my-zsh/" "${tmp_dir}/${pathless_zsh_dir}"/etc/skel/.zshrc

	# user cache
	sed -i "/^export ZSH=.*/a export ZSH_CACHE_DIR=~\/.oh-my-zsh\/cache" "${tmp_dir}/${pathless_zsh_dir}"/etc/skel/.zshrc

	# define theme
	sed -i 's/^ZSH_THEME=.*/ZSH_THEME="mrtazz"/' "${tmp_dir}/${pathless_zsh_dir}"/etc/skel/.zshrc

	# disable prompt while update
	sed -i 's/# DISABLE_UPDATE_PROMPT="true"/DISABLE_UPDATE_PROMPT="true"/g' "${tmp_dir}/${pathless_zsh_dir}"/etc/skel/.zshrc

	# disable auto update since we provide update via package
	sed -i 's/# DISABLE_AUTO_UPDATE="true"/DISABLE_AUTO_UPDATE="true"/g' "${tmp_dir}/${pathless_zsh_dir}"/etc/skel/.zshrc

	# define default plugins
	sed -i 's/^plugins=.*/plugins=(evalcache git git-extras debian tmux screen history extract colorize web-search docker)/' "${tmp_dir}/${pathless_zsh_dir}"/etc/skel/.zshrc

	chmod 755 "${tmp_dir}/${pathless_zsh_dir}"/DEBIAN/postinst

	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "${tmp_dir}/${pathless_zsh_dir}" >> "${DEST}"/${LOG_SUBPATH}/output.log 2>&1
	rsync --remove-source-files -rq "${tmp_dir}/${pathless_zsh_dir}.deb" "${DEB_STORAGE}/"
	rm -rf "${tmp_dir}"
...
compile_plymouth-theme-pathless()
{
	...
	plymouth_theme_pathless_dir=pathless-plymouth-theme_${REVISION}_all
	display_alert "Building deb" "pathless-plymouth-theme" "info"

	mkdir -p "${tmp_dir}/${plymouth_theme_pathless_dir}"/{DEBIAN,usr/share/plymouth/themes/pathless}

        # set up control file
	cat <<- END > "${tmp_dir}/${plymouth_theme_pathless_dir}"/DEBIAN/control
		Package: pathless-plymouth-theme
		Version: $REVISION
		Architecture: all
		Maintainer: $MAINTAINER <$MAINTAINERMAIL>
		...
		Priority: optional
		Description: boot animation, logger and I/O multiplexer - pathless theme
	END

	cp "${EXTER}"/packages/plymouth-theme-pathless/debian/{postinst,prerm,postrm} \
		"${tmp_dir}/${plymouth_theme_pathless_dir}"/DEBIAN/
	chmod 755 "${tmp_dir}/${plymouth_theme_pathless_dir}"/DEBIAN/{postinst,prerm,postrm}

	#convert -resize 256x256 \
	#	"${EXTER}"/packages/plymouth-theme-pathless/pathless-logo.png \
	#	"${tmp_dir}/${plymouth_theme_pathless_dir}"/usr/share/plymouth/themes/pathless/bgrt-fallback.png

	# convert -resize 52x52 \
	#       "${EXTER}"/packages/plymouth-theme-pathless/spinner.gif \
	#       "${tmp_dir}/${plymouth_theme_pathless_dir}"/usr/share/plymouth/themes/pathless/animation-%04d.png

	convert -resize 52x52 \
		"${EXTER}"/packages/plymouth-theme-pathless/spinner.gif \
		"${tmp_dir}/${plymouth_theme_pathless_dir}"/usr/share/plymouth/themes/pathless/throbber-%04d.png

	cp "${EXTER}"/packages/plymouth-theme-pathless/watermark.png \
		"${tmp_dir}/${plymouth_theme_pathless_dir}"/usr/share/plymouth/themes/pathless/

	cp "${EXTER}"/packages/plymouth-theme-pathless/{bullet,capslock,entry,keyboard,keymap-render,lock}.png \
		"${tmp_dir}/${plymouth_theme_pathless_dir}"/usr/share/plymouth/themes/pathless/

	cp "${EXTER}"/packages/plymouth-theme-pathless/pathless.plymouth \
		"${tmp_dir}/${plymouth_theme_pathless_dir}"/usr/share/plymouth/themes/pathless/

	fakeroot dpkg-deb -b -Z${DEB_COMPRESS} "${tmp_dir}/${plymouth_theme_pathless_dir}" > /dev/null
	rsync --remove-source-files -rq "${tmp_dir}/${plymouth_theme_pathless_dir}.deb" "${DEB_STORAGE}/"
	rm -rf "${tmp_dir}"
}

compile_pathless-config()
{
	local tmpdir=${SRC}/.tmp/pathless-config_${REVISION}_all

	display_alert "Building deb" "pathless-config" "info"


	mkdir -p "${tmpdir}"/{DEBIAN,usr/bin/,usr/sbin/,usr/lib/pathless-config/}

	# set up control file
	cat <<-END > "${tmpdir}"/DEBIAN/control
	Package: pathless-config
	Version: $REVISION
	Architecture: all
	Maintainer: $MAINTAINER <$MAINTAINERMAIL>
	Replaces: pathless-bsp
	...
	Recommends: pathless-bsp
	Suggests: libpam-google-authenticator, qrencode, network-manager, sunxi-tools
	Section: utils
	Priority: optional
	Description: Pathless configuration utility
	END

	install -m 755 $EXTER/cache/sources/pathless-config/scripts/tv_grab_file $tmpdir/usr/bin/tv_grab_file
	install -m 755 $EXTER/cache/sources/pathless-config/debian-config $tmpdir/usr/sbin/pathless-config
	install -m 644 $EXTER/cache/sources/pathless-config/debian-config-jobs $tmpdir/usr/lib/pathless-config/jobs.sh
	install -m 644 $EXTER/cache/sources/pathless-config/debian-config-submenu $tmpdir/usr/lib/pathless-config/submenu.sh
	install -m 644 $EXTER/cache/sources/pathless-config/debian-config-functions $tmpdir/usr/lib/pathless-config/functions.sh
	install -m 644 $EXTER/cache/sources/pathless-config/debian-config-functions-network $tmpdir/usr/lib/pathless-config/functions-network.sh
	install -m 755 $EXTER/cache/sources/pathless-config/softy $tmpdir/usr/sbin/softy
	# fallback to replace pathless-config in BSP
	ln -sf /usr/sbin/pathless-config $tmpdir/usr/bin/pathless-config
...
# <board>: pathlesspcplus, pathlesszero ...
...
userpatch_create()
{
	...
	git add .
	git -c user.name='Pathless User' -c user.email='user@example.org' commit -q -m "Cleaning working copy"
	...
	if ! git diff-index --quiet --cached HEAD; then
		...
		if [[ -n $(git config user.email) ]]; then
			...
			git format-patch -1 HEAD --stdout --signature="Created with pathless build tools https://github.com/Tron-Z/pathless_system" > "${patch}"
...
overlayfs_wrapper()
{
	...
	if [[ $operation == wrap ]]; then
		local srcdir="$2"
		local description="$3"
		mkdir -p /tmp/overlay_components/ /tmp/pathless_build/
		...
		mergeddir=$(mktemp -d --suffix="_$description" --tmpdir="/tmp/pathless_build/")
