...
install_common()
{
	...
	EOF
		...
		if [[ -n $BOOTENV_FILE ]]; then
			if [[ -f $USERPATCHES_PATH/bootenv/$BOOTENV_FILE ]]; then
				cp "$USERPATCHES_PATH/bootenv/${BOOTENV_FILE}" "${SDCARD}"/boot/pathlessEnv.txt
			elif [[ -f $EXTER/config/bootenv/$BOOTENV_FILE ]]; then
				cp "${EXTER}/config/bootenv/${BOOTENV_FILE}" "${SDCARD}"/boot/pathlessEnv.txt
			fi
		fi

		# TODO: modify $bootscript_dst or pathlessEnv.txt to make NFS boot universal
		...
		[[ -n $OVERLAY_PREFIX && -f "${SDCARD}"/boot/pathlessEnv.txt && ($BRANCH =~ current|next || $BOARDFAMILY =~ "rockchip-rk3588"|"rockchip-rk356x"|"sun60iw2") ]] && \
			echo "overlay_prefix=$OVERLAY_PREFIX" >> "${SDCARD}"/boot/pathlessEnv.txt

		[[ -n $DEFAULT_OVERLAYS && -f "${SDCARD}"/boot/pathlessEnv.txt && ($BRANCH =~ current|next || $BOARDFAMILY =~ "rockchip-rk3588"|"rockchip-rk356x") ]] && \
			echo "overlays=${DEFAULT_OVERLAYS//,/ }" >> "${SDCARD}"/boot/pathlessEnv.txt

		[[ -n $BOOT_FDT_FILE && -f "${SDCARD}"/boot/pathlessEnv.txt ]] && \
			echo "fdtfile=${BOOT_FDT_FILE}" >> "${SDCARD}/boot/pathlessEnv.txt"
	...
	[[ "${BOOTCONFIG}" != "none" ]] && {
		if [[ "${REPOSITORY_INSTALL}" != *u-boot* ]]; then
			...
			UBOOT_VER=$(dpkg --info "${DEB_PATHLESS}/u-boot/${CHOSEN_UBOOT}_${REVISION}_${ARCH}.deb" | grep Descr | awk '{print $(NF)}')
			install_deb_chroot "${DEB_PATHLESS}/u-boot/${CHOSEN_UBOOT}_${REVISION}_${ARCH}.deb" "pathless"
		fi
	}
...
PRE_INSTALL_KERNEL_DEBS
	...
	[[ -n $KERNELSOURCE ]] && {
		if [[ "${REPOSITORY_INSTALL}" != *kernel* ]]; then
			...
			install_deb_chroot "${DEB_PATHLESS}/${CHOSEN_KERNEL}_${REVISION}_${ARCH}.deb" "pathless"

			if [[ -f ${DEB_PATHLESS}/${CHOSEN_KERNEL/image/dtb}_${REVISION}_${ARCH}.deb ]]; then
...
POST_INSTALL_KERNEL_DEBS
	...
	if [[ "${REPOSITORY_INSTALL}" != *pathless-desktop* ]]; then
		...
		if [[ $BUILD_DESKTOP == yes ]]; then
			install_deb_chroot "${CHOSEN_DESKTOP}" "pathless"
			# install display manager and PACKAGE_LIST_DESKTOP_FULL packages if enabled per board
			desktop_postinstall
		fi
	fi

	# install pathless-firmware
	if [[ "${REPOSITORY_INSTALL}" != *pathless-firmware* ]]; then
		if [[ -f ${DEB_STORAGE}/pathless-firmware_${REVISION}_all.deb ]]; then
			install_deb_chroot "${DEB_STORAGE}/pathless-firmware_${REVISION}_all.deb"
		fi
	else
		if [[ -f ${DEB_STORAGE}/pathless-firmware_${REVISION}_all.deb ]]; then
			install_deb_chroot "${DEB_PATHLESS}/pathless-firmware_${REVISION}_all.deb" "pathless"
		fi
	fi

	# install pathless-config
	if [[ "${PACKAGE_LIST_RM}" != *pathless-config* ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *pathless-config* ]]; then
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_STORAGE}/pathless-config_${REVISION}_all.deb"
			fi
		else
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_PATHLESS}/pathless-config_${REVISION}_all.deb" "pathless"
			fi
		fi
	fi

	# install pathless-zsh
	if [[ "${PACKAGE_LIST_RM}" != *pathless-zsh* ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *pathless-zsh* ]]; then
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "${DEB_STORAGE}/pathless-zsh_${REVISION}_all.deb"
			fi
		else
			if [[ $BUILD_MINIMAL != yes ]]; then
				install_deb_chroot "pathless-zsh" "remote"
			fi
		fi
	fi

	# install plymouth-theme-pathless
	if [[ $PLYMOUTH == yes && $BUILD_DESKTOP == yes && $RELEASE != buster ]]; then
		if [[ "${REPOSITORY_INSTALL}" != *plymouth-theme-pathless* ]]; then
			install_deb_chroot "${DEB_STORAGE}/pathless-plymouth-theme_${REVISION}_all.deb"
		else
			install_deb_chroot "pathless-plymouth-theme" "remote"
		fi
	fi
	...
	# freeze pathless packages
	if [[ $BSPFREEZE == yes ]]; then
		display_alert "Freezing Pathless packages" "$BOARD" "info"
	...
	# add pathless user
	chroot "${SDCARD}" /bin/bash -c "adduser --quiet --disabled-password --shell /bin/bash --home /home/${PL_USERNAME} --gecos ${PL_USERNAME} ${PL_USERNAME}"
	chroot "${SDCARD}" /bin/bash -c "(echo ${PL_PWD};echo ${PL_PWD};) | passwd "${PL_USERNAME}" >/dev/null 2>&1"
	for additionalgroup in sudo netdev audio video disk tty users games dialout plugdev input bluetooth systemd-journal ssh docker; do
	        chroot "${SDCARD}" /bin/bash -c "usermod -aG ${additionalgroup} ${PL_USERNAME} 2>/dev/null"
	done

	# fix for gksu in Xenial
	touch ${SDCARD}/home/${PL_USERNAME}/.Xauthority
	chroot "${SDCARD}" /bin/bash -c "chown ${PL_USERNAME}:${PL_USERNAME} /home/${PL_USERNAME}/.Xauthority"
	# set up profile sync daemon on desktop systems
	chroot "${SDCARD}" /bin/bash -c "which psd >/dev/null 2>&1"
	if [ $? -eq 0 ]; then
		echo -e "${PL_USERNAME} ALL=(ALL) NOPASSWD: /usr/bin/psd-overlay-helper" >> ${SDCARD}/etc/sudoers
		touch ${SDCARD}/home/${PL_USERNAME}/.activate_psd
		chroot "${SDCARD}" /bin/bash -c "chown $PL_USERNAME:$PL_USERNAME /home/${PL_USERNAME}/.activate_psd"
	fi
	...
	cp "${EXTER}"/packages/blobs/splash/pathless-u-boot.bmp "${SDCARD}"/boot/boot.bmp
	...
	# optional demo video (omit from slim tree)
	[[ -f "${EXTER}"/packages/blobs/test.mp4 ]] && cp "${EXTER}"/packages/blobs/test.mp4 "${SDCARD}"/usr/local/
...
FAMILY_TWEAKS
	...
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable pathless-firstrun.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable pathless-firstrun-config.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable pathless-zram-config.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable pathless-hardware-optimize.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable pathless-ramlog.service >/dev/null 2>&1"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable pathless-resize-filesystem.service >/dev/null 2>&1"
	...
	#[[ $IMAGE_TYPE == nightly ]] \
	#&& echo "deb http://beta.pathless.com $RELEASE main ${RELEASE}-utils ${RELEASE}-desktop" \
	#> "${SDCARD}"/etc/apt/sources.list.d/pathless.list
	...
	# save initial pathless-release state
	cp "${SDCARD}"/etc/pathless-release "${SDCARD}"/etc/pathless-image-release
...
install_distribution_specific()
{
	...
	bionic|focal|hirsute|impish|jammy|noble)
			...
			[[ -d "${SDCARD}"/etc/netplan ]] && cat <<-EOF > "${SDCARD}"/etc/netplan/pathless-default.yaml
			network:
			  version: 2
			  renderer: $RENDERER
			EOF
...
install_pathless_specific()
{
	...
	cp -rfa ${EXTER}/cache/sources/pathless-firmware-git/* ${SDCARD}/lib/firmware/
	...
	[[ -n $OVERLAY_PREFIX && -f "${SDCARD}"/boot/pathlessEnv.txt && ($BRANCH =~ current|next || $BOARDFAMILY =~ "rockchip-rk3588"|"rockchip-rk356x") ]] && \
		echo "overlay_prefix=$OVERLAY_PREFIX" >> "${SDCARD}"/boot/pathlessEnv.txt
	...
	cat <<-EOF > "${SDCARD}"/etc/pathless-release
	...
	install -m 755 "${EXTER}/packages/raspi/pathless/common/files/pathless-hardware-optimization" "${SDCARD}/usr/lib/pathless/"
	install -m 755 "${EXTER}/packages/raspi/pathless/common/files/pathless-hardware-optimize.service" "${SDCARD}/usr/lib/systemd/system/"
	chroot "${SDCARD}" /bin/bash -c "systemctl --no-reload enable pathless-hardware-optimize.service >/dev/null 2>&1"
