...
create_board_package()
{
	...
	if [[ "${BOOTCONFIG}" != "none" ]]; then
		...
		mkdir -p "${destination}"/usr/share/pathless/

		# create extlinux config file
		if [[ $SRC_EXTLINUX != yes ]]; then
			if [ -f "${USERPATCHES_PATH}/bootscripts/${bootscript_src}" ]; then
			  cp "${USERPATCHES_PATH}/bootscripts/${bootscript_src}" "${destination}/usr/share/pathless/${bootscript_dst}"
			else
			  cp "${EXTER}/config/bootscripts/${bootscript_src}" "${destination}/usr/share/pathless/${bootscript_dst}"
			fi
			[[ -n $BOOTENV_FILE && -f $SRC/config/bootenv/$BOOTENV_FILE ]] && \
				cp "${EXTER}/config/bootenv/${BOOTENV_FILE}" "${destination}"/usr/share/pathless/pathlessEnv.txt
		fi
	...
	# Depends: fping is needed for pathlessmonitor to upload pathless-hardware-monitor.log
	...
	Suggests: pathless-config
	Replaces: zram-config, base-files, pathless-tools-$RELEASE, linux-${RELEASE}-root-legacy-$BOARD (<< $REVISION~), linux-${RELEASE}-root-current-$BOARD (<< $REVISION~), linux-${RELEASE}-root-next-$BOARD (<< $REVISION~)
	...
	Description: Pathless board support files for $BOARD
	EOF
	...
	*)
	...
	[ -f "/usr/lib/pathless/apt-updates" ] && rm /usr/lib/pathless/apt-updates
	[ -f "/usr/lib/pathless/firstrun-config.sh" ] && rm /usr/lib/pathless/firstrun-config.sh
	...
	if [ remove = "\$1" ] || [ abort-install = "\$1" ]; then
	    ...
	    systemctl disable pathless-hardware-monitor.service pathless-hardware-optimize.service >/dev/null 2>&1
	    systemctl disable pathless-zram-config.service pathless-ramlog.service >/dev/null 2>&1
	...
	[ -f /etc/lib/systemd/system/pathless-ramlog.service ] && systemctl --no-reload enable pathless-ramlog.service
	...
	if [ -n "\$(grep -w '^ENABLED=false' /etc/default/log2ram 2> /dev/null)" ]; then
	     ...
	     sed -i "s/^ENABLED=.*/ENABLED=false/" /etc/default/pathless-ramlog
	...
	cat <<-EOF >> "${destination}"/DEBIAN/postinst
    # move bootscript to /usr/share/pathless
    # create a backup
    [ -f /etc/pathless-release ] &&  . /etc/pathless-release
    [ -z \${VERSION} ] && VERSION=$(echo \`date +%s\`)
    if [ -f /boot/$bootscript_dst ]; then
       cp /boot/$bootscript_dst /usr/share/pathless/${bootscript_dst}-\${VERSION} >/dev/null 2>&1
       echo "NOTE: You can find previous bootscript versions in /usr/share/pathless !"
    fi

    # cleanup old bootscript backup
    ls /usr/share/pathless/boot.cmd-* >/dev/null 2>&1 | head -n -5 | xargs rm -f --
    ls /usr/share/pathless/boot.ini-* >/dev/null 2>&1 | head -n -5 | xargs rm -f --

    echo "Recreating boot script"
    cp /usr/share/pathless/$bootscript_dst /boot  >/dev/null 2>&1
    rootdev=\$(sed -e 's/^.*root=//' -e 's/ .*\$//' < /proc/cmdline)
    rootfstype=\$(sed -e 's/^.*rootfstype=//' -e 's/ .*$//' < /proc/cmdline)

    # recreate pathlessEnv.txt if it and extlinux does not exists
    if [ ! -f /boot/pathlessEnv.txt ] && [ ! -f /boot/extlinux/extlinux.conf ]; then
      cp /usr/share/pathless/pathlessEnv.txt /boot  >/dev/null 2>&1
      echo "rootdev="\$rootdev >> /boot/pathlessEnv.txt
      echo "rootfstype="\$rootfstype >> /boot/pathlessEnv.txt
    fi
	...
	if [ ! -f "/etc/default/pathless-motd" ]; then
		mv /etc/default/pathless-motd.dpkg-dist /etc/default/pathless-motd
	fi
	if [ ! -f "/etc/default/pathless-ramlog" ] && [ -f /etc/default/pathless-ramlog.dpkg-dist ]; then
		mv /etc/default/pathless-ramlog.dpkg-dist /etc/default/pathless-ramlog
	fi
	if [ ! -f "/etc/default/pathless-zram-config" ] && [ -f /etc/default/pathless-zram-config.dpkg-dist ]; then
		mv /etc/default/pathless-zram-config.dpkg-dist /etc/default/pathless-zram-config
	fi
	...
	# Reload services
	systemctl --no-reload enable pathless-hardware-monitor.service pathless-hardware-optimize.service pathless-zram-config.service >/dev/null 2>&1
	exit 0
	EOF
	...
	for i in ${releases[@]}
	do
		echo "$(echo $i | sed 's/.*\///')=$(cat $i/support)" >> "${destination}"/etc/pathless-distribution-status
	done

	# armhwinfo, firstrun, pathlessmonitor, etc. config file
	cat <<-EOF > "${destination}"/etc/pathless-release
...
POST_FAMILY_TWEAKS_BSP
	...
	fingerprint_image "${destination}/etc/pathless.txt"
