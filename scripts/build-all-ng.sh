...
# cleanup
rm -f /run/pathless/*.pid
mkdir -p /run/pathless
...
build_main ()
{
	...
	touch "/run/pathless/${BOARD^}_${BRANCH}_${RELEASE}_${BUILD_DESKTOP}_${BUILD_MINIMAL}.pid";
	...
	echo -e "\n"
	rm "/run/pathless/${BOARD^}_${BRANCH}_${RELEASE}_${BUILD_DESKTOP}_${BUILD_MINIMAL}.pid"
}
	...
	while read -r line; do
		...
		if [[ "${BUILD_STABILITY}" == "${STABILITY}" ]]; then
			...
			if [[ $1 != "dryrun" ]] && [[ $n -ge $START ]]; then
				...
				while :
				do
					[[ $(find /run/pathless/*.pid 2>/dev/null | wc -l) -le ${MULTITHREAD} || -z ${MULTITHREAD} ]] && break
					sleep 5
				done
...
while :
do
		if [[ -z $(ps -uax | grep 7z | grep Pathless) ]]; then
			break
		fi
	sleep 5
done
