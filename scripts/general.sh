...
cleaning()
{
	case $1 in
		...
		if [[ -d "${DEB_STORAGE}" ]]; then
			...
			[[ -n $RELEASE ]] && rm -f "${DEB_STORAGE}/${RELEASE}/pathless-desktop-${RELEASE}"_*.deb
		fi
		;;
...
display_alert "Building kernel splash logo" "$RELEASE" "info"
	...
	"${SDCARD}"/lib/firmware/bootsplash.pathless >/dev/null 2>&1
	if [[ $BOOT_LOGO == yes || $BOOT_LOGO == desktop && $BUILD_DESKTOP == yes && $RELEASE != buster ]]; then
		[[ -f "${SDCARD}"/boot/pathlessEnv.txt ]] &&	grep -q '^bootlogo' "${SDCARD}"/boot/pathlessEnv.txt && \
		sed -i 's/^bootlogo.*/bootlogo=true/' "${SDCARD}"/boot/pathlessEnv.txt || echo 'bootlogo=true' >> "${SDCARD}"/boot/pathlessEnv.txt
...
function set_distribution_status() {
	...
	[[ "${DISTRIBUTION_STATUS}" != "supported" ]] && [[ "${EXPERT}" != "yes" ]] && exit_with_error "Pathless ${RELEASE} is unsupported and, therefore, only available to experts (EXPERT=yes)"
...
# Installing debian packages in the pathless build system.
...
prepare_host()
{
	...
	*run after all host dependencies are installed*
	...
	if [[ ! -f "${USERPATCHES_PATH}"/README ]]; then
		...
		echo 'https:/github.com/Tron-Z' >> "${USERPATCHES_PATH}"/README
...
get_pathless_url()
{
	if [[ ${GITEE_SERVER} == yes ]]; then
		echo "https://gitee.com/Tron-Z"
	else
		echo "https://github.com/Tron-Z"
	fi
}
...
install_wiringop()
{
	...
	if [[ ${IGNORE_UPDATES} != yes ]]; then
		...
		fetch_from_repo "${PATHLESS_WIRINGOP_REPO:-${PATHLESS_SYSTEM_REPO}}" "${EXTER}/cache/sources/wiringOP" "${PATHLESS_WIRINGOP_BRANCH:-branch:wiringOP}" "yes"
		fetch_from_repo "${PATHLESS_WIRINGOP_PYTHON_REPO:-${PATHLESS_SYSTEM_REPO}}" "${EXTER}/cache/sources/wiringOP-Python" "${PATHLESS_WIRINGOP_PYTHON_BRANCH:-branch:wiringOP-Python}" "yes"
...
install_310b-npu-driver()
{
	...
	if [[ -f "${driver}" ]]; then
		...
		chroot "${SDCARD}" /bin/bash -c "/opt/${driver_name} --chroot --full --install-username=pathless --install-usergroup=pathless --install-for-all"
	fi
}
...
#		[[ -z $PASS_ROOT ]] && PASS_ROOT="pathless"
...
#				sshpass -p ${PASS_ROOT} ssh root@${REMOTEIP} "dd bs=1k seek=8 if=/usr/lib/linux-u-boot-legacy-pathless400_2.1.0_arm64/boot0_sdcard.fex of=/dev/mmcblk0"
#				sshpass -p ${PASS_ROOT} ssh root@${REMOTEIP} "dd bs=1k seek=16400 if=/usr/lib/linux-u-boot-legacy-pathless400_2.1.0_arm64/boot_package.fex of=/dev/mmcblk0"
...
#			if [[ $BRANCH == current && $BOARD =~ pathlesszero2|pathless400 ]]; then
#				sshpass -p ${PASS_ROOT} scp ${LINUXSOURCEDIR}/arch/arm64/boot/dts/allwinner/sun50i-h616-pathless-*.dtb root@${REMOTEIP}:/boot/dtb/allwinner/
#			fi
#
#			sshpass -p ${PASS_ROOT} ssh root@${REMOTEIP} "sync"
#			sshpass -p ${PASS_ROOT} ssh root@${REMOTEIP} "reboot"
