...
sudo sed -i '/extraargs=fbcon=rotate/d' /boot/pathlessEnv.txt
sudo bash -c "echo extraargs=fbcon=rotate:${rotate} >> /boot/pathlessEnv.txt"
sudo sed -i 's/bootlogo=true/bootlogo=false/g' /boot/pathlessEnv.txt
sudo sync
